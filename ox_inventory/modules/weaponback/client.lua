...
local function handleEntityAttached(entity, stateData)
    ...
    AttachEntityToEntity(entity, playerPed, boneIndex, attachPos.x, attachPos.y, attachPos.z, attachRot.x, attachRot.y,
        attachRot.z, false, false, false, false, 2, true)
...
function CubXWeaponBack.Toggle(item, model)
    ...
    lib.callback("ox_inventory:toggleWeaponBack", false, function(response)
        ...
        if response.action == "attached" then
            print(string.format("^2[weaponback] %s attached on back (back slot %d)^7", tostring(item.name),
                response.back or -1))
