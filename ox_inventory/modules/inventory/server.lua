...
function Inventory.SlotWeight(item, slot, ignoreCount)
	if item.weapon then
		...
		if PermanentMod and PermanentMod.isWeapon(slot.name) then
			return 0
		end
	end
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if (activeSlots[fromRef] and (currentTime - activeSlots[fromRef]) < 1000) or (activeSlots[toRef] and (currentTime - activeSlots[toRef]) < 1000) then
		return false, {
			{
	...
	activeSlots[fromRef] = currentTime
	activeSlots[toRef] = currentTime
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		do
			local PermanentMod = package.loaded['modules.permanent.server']

			if PermanentMod and PermanentMod.checkSwapItems then
				local toType = data.toType == 'newdrop' and 'drop' or data.toType
				local result = PermanentMod.checkSwapItems({
					fromSlot = fromData,
	...
	if type == 'table' and action == 'component' then
		...
		if weapon and weapon.metadata then
			...
			if PermanentMod then
				isPermanent = PermanentMod.isWeapon(weapon.name)
			end

			if action == 'load' and (isPermanent or weapon.metadata.durability > 0) then
			...
			elseif action == 'melee' and value > 0 and not isPermanent then
