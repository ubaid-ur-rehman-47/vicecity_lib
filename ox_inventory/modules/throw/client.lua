...
function CubXThrow.Start(item, itemModel)
    ...
    CreateThread(function()
        while isAiming do
            ...
            local projectile = GetClosestObjectOfType(pedCoords.x, pedCoords.y, pedCoords.z, 100.0, -536771815, false,
                false, false)
            ...
            while DoesEntityExist(projectile) do
                ...
                projectile = GetClosestObjectOfType(pedCoords.x, pedCoords.y, pedCoords.z, 100.0, -536771815, false,
                    false, false)
            end
    ...
    CreateThread(function()
        ...
        while isThrowActive do
            ...
            if IsPedShooting(ped) then
                ...
                if spinForce > 0 then
                    ApplyForceToEntity(newProp, 1, (math.random() - 0.5) * spinForce, (math.random() - 0.5) * spinForce,
                        (math.random() - 0.5) * spinForce, 0.0, 0.12, 0.05, 0, false, true, true, false, true)
                end
                ...
                CreateThread(function()
                                    ...
                                    if playerPed ~= ped then
                                        if IsEntityTouchingEntity(newProp, playerPed) then
                                            if not IsPedRagdoll(playerPed) then
                                                TriggerServerEvent("ox_inventory:throwHit", GetPlayerServerId(playerId),
                                                    propVel)
                                                break
                                            end
                                        end
                    ...
                    TriggerServerEvent("ox_inventory:throwItem", item.slot, finalCoords, model)
                end)
...
local function handlePickup(dropId)
    ...
    SetTimeout(delay, function()
        lib.callback("ox_inventory:pickupThrownDrop", false, function(success)
            isPickingUp = false
            ClearPedTasks(cache.ped)
            if not success then
                lib.notify({
...
local function addTargetZone(dropId, coords)
    ...
    local zoneId = exports.ox_target:addSphereZone({
        ...
        options = {
            {
                name = string.format("ox_inventory:throwPickup_%s", dropId),
...
RegisterNetEvent("ox_inventory:throwHit", function(velocity)
                    ...
                    if intType == "text3d" then
                        ...
                        for dropId, dropInfo in pairs(activeDrops) do
                            ...
                            if dist <= text3dConfig.maxDistance then
                                local drawCoords = vec3(dropInfo.coords.x, dropInfo.coords.y,
                                    dropInfo.coords.z + text3dConfig.zOffset)
