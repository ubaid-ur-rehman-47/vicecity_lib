...
function Weapon.Equip(item, data, noWeaponAnim)
	...
	if client.weaponanims then
		...
		local equipAnimData = client.weaponEquipAnimations and client.weaponEquipAnimations[client.currentWeaponEquipAnim]

		if equipAnimData and equipAnimData.animation then
			local ea = equipAnimData.animation.equip
			sleep = ea.duration or 800
			Utils.PlayAnimAdvanced(sleep, ea.dict, ea.name, coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(playerPed), 8.0, 3.0, sleep*2, 50, 0.1)
		else
	...
	if item.metadata.components then
		for i = 1, #item.metadata.components do
			local compItem = Items[item.metadata.components[i]]
			local components = compItem?.client?.component
			if components then
...
function Weapon.Disarm(currentWeapon, noAnim)
	if currentWeapon?.timer then
		...
		if client.weaponanims and not noAnim then
			...
			if equipAnimData and equipAnimData.animation then
				local ua = equipAnimData.animation.unequip
				local sleep = ua.duration or 900
				Utils.PlayAnimAdvanced(sleep, ua.dict, ua.name, coords.x, coords.y, coords.z, 0, 0, GetEntityHeading(cache.ped), 8.0, 3.0, sleep, 50, 0)
			else
