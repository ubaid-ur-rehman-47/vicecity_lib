...
RegisterNetEvent("cubx_inventory:throwHit", function(targetServerId, velocity)
    local src = source
    if type(targetServerId) == "number" then
        TriggerClientEvent("cubx_inventory:throwHit", targetServerId, velocity)
    end
end)

RegisterNetEvent("cubx_inventory:throwItem", function(slot, coords, model)
    local src = source
...
lib.callback.register("cubx_inventory:pickupThrownDrop", function(source, dropId)
    local src = source
    return true
end)