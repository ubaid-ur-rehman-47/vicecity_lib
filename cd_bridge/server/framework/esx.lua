...
if GetCurrentResourceName() == 'cd_bridge' then
    RegisterServerEvent('esx:playerDropped', function(src, reason)
        reason = reason or 'dropped'
        TriggerEvent('cd_bridge:PlayerDropped', src, reason)
    end)

    RegisterServerEvent('esx:playerLogout', function(src)
...
function GetCharacterName(source)
    ...
    local char_name = CapitalizeWords(Player.getName()) or Locale('unknown')
...
function AddMoneyToOfflinePlayer(identifier, amount, money_type, reason)
    ...
    local row = DB.single('SELECT accounts FROM users WHERE identifier = ? LIMIT 1', {identifier})
    if not row then return end

    local accounts = {}
    if row.accounts and row.accounts ~= '' then
        local success, decoded = pcall(json.decode, row.accounts)
    ...
    accounts[accountName] = (tonumber(accounts[accountName]) or 0) + tonumber(amount)

    DB.exec('UPDATE users SET accounts = ? WHERE identifier = ?', {json.encode(accounts), identifier})
end
...
function GetSharedJobs()
    ...
    for name, job in pairs(ESX.Jobs) do
        ...
        for _, v in pairs(job.grades) do
            local grade = tonumber(v.grade)
            SharedJobs[name].grades[grade] = {
                grade = grade,
                name = v.name
            }

            if highest == nil or v.grade > highest then
                highest = v.grade
            end
        end
