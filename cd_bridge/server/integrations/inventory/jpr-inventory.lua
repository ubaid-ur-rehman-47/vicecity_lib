if Cfg.Inventory ~= 'jpr-inventory' then return end
...
function AddItem(source, item_name, amount, metadata)
    return exports['jpr-inventory']:AddItem(source, item_name, amount, false, metadata, '')
end

-- Remove item from a player's inventory
function RemoveItem(source, item_name, amount, slot)
    return exports['jpr-inventory']:RemoveItem(source, item_name, amount, slot, '')
end

-- Check if a player has a specific item and amount
function HasItem(source, item_name, amount)
    amount = amount or 1
    return exports['jpr-inventory']:HasItem(source, item_name, amount)
end

-- Get the count of a specific item in a player's inventory
function GetItemCount(source, item_name)
    local item_count = exports['jpr-inventory']:GetItemCount(source, item_name)
    if item_count then
        return item_count
    end
    return 0
...
function CanCarryItem(source, item_name, amount)
    return exports['jpr-inventory']:CanAddItem(source, item_name, amount)
end
...
function GetPlayerInventory(source)
    local inventory = exports['jpr-inventory']:GetInventory(source)
...
-- Add quality to an item in a specific slot
function AddQualityToItem(source, slot, qualityIncrease, itemName)
    ...
    exports['jpr-inventory']:SetItemData(source, itemName, 'info', metadata)
end

-- Remove quality from an item in a specific slot
...
-- Set the quality of an item in a specific slot
...
function GetItemList()
    ...
    local items = exports['jpr-inventory']:GetItemList()
    if not items then
        return ItemList
    end
...
function GetInventoryImages()
    ...
    local images = exports.cd_bridge:ReadNUIDirectory(
        GetResourcePath('jpr-inventory')..'/html/images',
        'jpr-inventory/html/images/',
